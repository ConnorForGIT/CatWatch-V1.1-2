...
- (id)init //默认初始化为全屏
{
    CGRect frame = {{0,0},{self.frame.size.width,self.frame.size.height}};
...
- (void)addImgURL:(NSString *)url //添加图片数据
{
    if (url != nil) {
        [_imageArray addObject:url];
    }
}
- (void)addModelSource:(ModelSource *)model
{
    if (model != nil) {
...
- (void)loadScrollView //加载滚动视图数据
{
    ...
    for (int i = 0; i < _imageArray.count; i++) {
        if ([[_imageArray[i] typeStr] isEqualToString:@"1"]) {
    ...
    for(int i=0; i< _imageArray.count; i++)
    {
        ...
        if (![[_imageArray[i] typeStr] isEqualToString:@"1"]) {
            NSURL * url = [NSURL URLWithString:[[_imageArray objectAtIndex:i] cover_landscapeURL]];
        ...
        textView.frame = CGRectMake(0, imageView.frame.size.height - 55, imageView.frame.size.width, 55);
        textView.backgroundColor = [UIColor blackColor];
        textView.alpha = 0.6;
        
        // 标题
        UILabel * textTitle = [[UILabel alloc]init];
        textTitle.frame = CGRectMake(textView.frame.origin.x + 12, 5, textView.frame.size.width - 25, 20);
        textTitle.textAlignment = NSTextAlignmentCenter;
        textTitle.textColor = [UIColor orangeColor];
        textTitle.font = [UIFont boldSystemFontOfSize:16];
        
        // 作者
        UILabel * textAuth = [[UILabel alloc]init];
        textAuth.frame = CGRectMake(textTitle.frame.origin.x, textTitle.frame.origin.y + textTitle.frame.size.height + 5, 100, 20);
        textAuth.textColor = [UIColor grayColor];
        textAuth.font = [UIFont boldSystemFontOfSize:14];
        
        // 日期
        UILabel * textDate = [[UILabel alloc]init];
        textDate.frame = CGRectMake(textAuth.frame.origin.x + textAuth.frame.size.width + 10, textAuth.frame.origin.y, 140, 20);
        textDate.textAlignment = NSTextAlignmentRight;
        textDate.textColor = [UIColor lightGrayColor];
        textDate.font = [UIFont boldSystemFontOfSize:14];
        
        
        [textView addSubview:textDate];
        [textView addSubview:textTitle];
        [textView addSubview:textAuth];
        ...
        textTitle.text = [_imageArray[i] titleStr];
        textAuth.text = [_imageArray[i] source];
        textDate.text = [_imageArray[i] pubdateStr];
        
        
        
        imageView.layer.transform = CATransform3DMakeRotation(_angleValue, 0, 0, 0);//设置角度
        if(i != 0 )
...
- (void) scrollViewDidScroll:(UIScrollView *)scrollView //滚动时处理
{
    ...
    for(int i=0; i<_imageViewArray.count; i++)
    {
        ...
        scrollImageView.layer.transform = CATransform3DMakeRotation(angleValue, 0, 0, 0); //改变角度
