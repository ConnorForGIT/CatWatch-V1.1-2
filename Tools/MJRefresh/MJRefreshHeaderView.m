...
- (UILabel *)lastUpdateTimeLabel
{
    if (!_lastUpdateTimeLabel) {
        ...
        //动画
        UIImageView *aimage = [[UIImageView alloc]initWithFrame:CGRectMake(30, 5, 50, 50)];
        [self addSubview:aimage];
                NSMutableArray *arr = [NSMutableArray array];
        for (int i = 0; i < 4; i++) {
            UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"000%d.png",i + 1]];
            [arr addObject:image];
        }
        [aimage setCenter:[aimage center]];
        //设置动画数组
        [aimage setAnimationImages:arr];
        //设置动画时长
        [aimage setAnimationDuration:0.2];
        //开始动画
